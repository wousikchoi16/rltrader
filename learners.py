...
class ReinforcementLearner:
    ...
    def reset(self): #Learners.run()에서만 쓰인다.
        self.sample = None
        self.training_data_idx = -1
        # 환경 초기화
    ...
    @abc.abstractmethod
    def get_batch(self): # return (x, y_value, y_policy)
        pass
    ...
    def visualize(self, epoch_str, num_epoches, epsilon):
        ...
        self.visualizer.save(os.path.join(
            ...
            'epoch_summary_{}.png'.format(epoch_str)) # self.epoch_summary_dir는 run()실행시 만들어짐. 
        )
    ...
    def run(self, learning=True):
        ...
        for epoch in tqdm(range(self.num_epoches)):
            ...
            for i in tqdm(range(len(self.training_data))):
                ...
                # 반복에 대한 정보 갱신
                self.batch_size += 1 # 초기화=0, 초기화이외에는 여기서만 쓰임. 
                self.itr_cnt += 1
...
class DQNLearner(ReinforcementLearner):
    ...
    def get_batch(self):
        ...
        x = np.zeros((len(self.memory_sample), self.num_steps, self.num_features))  #(갯수,2?,28)
        y_value = np.zeros((len(self.memory_sample), self.agent.NUM_ACTIONS))       #(갯수,2)
        value_max_next = 0
