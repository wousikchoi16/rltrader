...
def preprocess(data, ver='v1'): #load_data에서만 쓰임. 
    windows = [5, 10, 20, 60, 120]
    for window in windows: # 나중에 f-string으로 바꾸자
    ...
    if ver == 'v1.rich':
        ...
        data.loc[1:, 'inst_lastinst_ratio'] = (
            ...
            / data['inst'][:-1].replace(to_replace=0, method='ffill').replace(to_replace=0, method='bfill').values # replace(to_replace=0, method='ffill'): 0값은 이전값으로 바꿈. 
        )
...
def load_data(code, date_from, date_to, ver='v2'):
    ...
    # 기간 필터링
    data['date'] = data['date'].str.replace('-', '') # v1파일만 해당됨. 
    data = data[(data['date'] >= date_from) & (data['date'] <= date_to)]
    data = data.dropna()

    # 차트 데이터 분리
    chart_data = data[COLUMNS_CHART_DATA] # 날짜 OHLCV (총6항목)
...
def load_data_v3(code, date_from, date_to): # 향후 개발
    df = None
